...
#
# Purpose: Run race-stratified analysis 
#
...
race <- as.character(args[3]) # {black, white, hisp}


# Read in data ------------------------------------------------------------

dat <- read_csv(paste0("./screening/data/screen_", race, ".csv"), 
...
# IP-weights --------------------------------------------------------------
  ...
  # For HIV status
  mod_hiv <- glm(as.factor(hiv_base) ~ bs(age_start, df=3) + sex + first_elig_state + enroll_period, 
  ...
  boot$wt_screen <- rep(NA, nrow(boot))
    ...
    mod_screen <- glm(as.factor(screen) ~ bs(age_start, df=3) + screen_1 + screen_2 +  
                      sex + first_elig_state + n_cond + n_cond_1 + enroll_period,
...
if (nboot==0) {
  ...
  write_csv(est, paste0("./screening/results/screen_res_", def, "yr_", race, ".csv"))
}
