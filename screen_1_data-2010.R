...
#
# Purpose: Manage raw data and restrict to 2010-2015
#
...
set.up <- function(split) {
  ...
  dat2 <- dat2 %>% 
    filter(event_date > ymd("2010-01-01")) %>% 
    mutate(baseline = ifelse(baseline>ymd("2010-01-01"), baseline, ymd("2010-01-01")),
           baseline = as.Date(baseline, origin=ymd("1970-01-01")))
...
write_csv(all.dat, "./screening/data/screen_post2010.csv")
