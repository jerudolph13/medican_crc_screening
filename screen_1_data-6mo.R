...
#
# Purpose: Manage raw data - related 6mo sensitivity analysis
#
...
proc <- read_csv(file="../../data/MedicanDataRequest/jackie/20230622_jackie_crc_v4/crc_file2.csv",
                 col_types=c("cfD")) %>% 
  filter(proc_type!=3) # Exclude barium enema
...
# If endoscopies are within 6 months, they are "related"
...
proc3 <- proc2 %>% 
  ...
  filter(unique==1 | cum_t_between>0.5) 
...
repeat {
  proc3 <- proc3 %>%   
    mutate(unique = ifelse(lag(unique)==1 | is.na(lag(unique)), 1, 0),
           ...
           cum_t_between = cumsum(t_between)) %>% 
    filter(unique==1 | cum_t_between>0.5)
  temp <- filter(proc3, unique==0 & t_between<0.5)
  n <- nrow(temp)
  if (n==0) {break}
}  
...
dat4 <- bind_rows(lapply(dat3, rep, dat3$event_int)) %>%
  ...
  mutate(# Set up time
    ...
    int_start = ifelse(is.na(int_start), 
                       ...
                       int_start),
    int_start = as.Date(int_start, origin="1970-01-01"),
    ...
    int_end = as.Date(int_end, origin="1970-01-01"),
...
censor <- dat4 %>% 
  ...
  filter(int_end<(dob + years(65) - months(1)) & int_end<mdy("09-30-2015")) %>% # But not admin censoring
  mutate(int = int + 1,
         int_start = int_end,
         age_start = age_end,
...
dat7 <- left_join(dat6, date.fill, by=c("bene_id", "int")) %>% 
  mutate(date_last_endo = ifelse(endo==1, lag(date_last_endo), date_last_endo),
         date_last_endo = as.Date(date_last_endo, origin="1970-01-01"),
...
write_csv(all.dat, "./screening/data/screen_related-screen-6mo.csv")
