...
if (analysis=="main") {
  ...
  dat <- read_csv("./screening/data/screen_post2010.csv", 
                  col_types=c("cDDDfdfffffDdddfDdDdffdfDdf"))
} else if (analysis=="related6mo") {
  dat <- read_csv("./screening/data/screen_related-screen-6mo.csv", 
                  col_types=c("cDDDfdfffffDdddfDdDdffdfDdf"))
} else if (analysis=="colonoscopy") {
  dat <- read_csv("./screening/data/screen_colonoscopy.csv", 
                  col_types=c("cDDDfdfffffDdddfDdDdffdfDdf"))
}
...
# IP-weights --------------------------------------------------------------
  ...
  # For HIV status
  mod_hiv <- glm(as.factor(hiv_base) ~ bs(age_start, df=3) + race + sex + first_elig_state + enroll_period, 
  ...
  boot$wt_screen <- rep(NA, nrow(boot))
    ...
    mod_screen <- glm(as.factor(screen) ~ bs(age_start, df=3) + screen_1 + screen_2 +  
                      race + sex + first_elig_state + n_cond + n_cond_1 + enroll_period,
...
if (analysis=="main") {
  if (nboot==0) {
    ...
    write_csv(est, paste0("./screening/results/screen_res_", def, "yr.csv"))
  }
} else if (analysis=="post2010") {
  write_csv(est, paste0("./screening/results/screen_res_post2010-", def, "yr.csv"))
} else if (analysis=="related6mo") {
  write_csv(est, paste0("./screening/results/screen_res_related6mo-", def, "yr.csv"))
} else if (analysis=="colonoscopy") {
  write_csv(est, paste0("./screening/results/screen_res_colonoscopy-", def, "yr.csv"))
}
